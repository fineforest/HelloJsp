...
<head>
    ...
    <title>사원정보 리스트 페이지(ResultSet)</title>
	<%----------------------------------------------------------------------
...
<%!
	...
	//사원정보 테이블 헤더
	public String[] SawonHead = { "사원", "성명", "나이", "성별", "전화",
								  "부서", "부서명", "직급", "직급명", "생일", "주소" };
...
<%
	...
	String  sJobStatus	= null;								// 파라미터 : 작업상태 - 사원정보 데이터베이스 작업상태(SELECT)
	...
	ArrayList<SawonDTO> Sawons = new ArrayList<SawonDTO>();	// 사원정보 검색 결과용 ArrayList 객체 
	// ---------------------------------------------------------------------
	// [웹 페이지 get/post 파라미터 조건 필터링]
	// ---------------------------------------------------------------------
	bJobProcess	= ComMgr.IsNull(request.getParameter("JobProcess"), false);		// 파라미터 : 작업처리 - null 확인(false : 아무동작 없음)
	sJobStatus	= ComMgr.IsNull(request.getParameter("jobstatus"), "SELECT");	// 파라미터 : 작업상태 - null 확인(SELECT : 사원정보 검색)
	sAge		= ComMgr.IsNull(request.getParameter("age"), 0).toString();		// 파라미터 : 나이 - null 확인(0 : 전체)
	sGender		= ComMgr.IsNull(request.getParameter("gender"), "-1");			// 파라미터 : 성별 - null 확인(-1 : 전체)
	sDept		= ComMgr.IsNull(request.getParameter("dept"), "-1");			// 파라미터 : 부서 - null 확인(-1 : 전체)
	// ---------------------------------------------------------------------
	// [일반 변수 조건 필터링]
	// ---------------------------------------------------------------------
...
------------------------------------------------------------------------------%>
	...
	--------------------------------------------------------------------------%>
<%
	// JobStatus 작업처리 허용인 경우 검색을 위한 조건을 DTO에 넣기
	if (bJobProcess == true)
	{
...
------------------------------------------------------------------------------%>
<%
	// JobStatus 작업처리 허용 && 검색인 경우 사원정보 검색
	if (bJobProcess == true && sJobStatus.equals("SELECT") == true)
	{
		if (this.sawonDAO.ReadSawonList(SawonDTO) == true)
		{
...
<body class="Body">
	...
	--------------------------------------------------------------------------%>
	<form name="form1" action="SawonList_Rs.jsp?JobProcess=true&jobstatus=SELECT" method="post">
		<%------------------------------------------------------------------
			Top 고정 영역
		----------------------------------------------------------------------%>
		<div class="Top-Fixed">
			...
			----------------------------------------------------------------------%>
			<hr  class="Line">
			<div class="Title">JSP 사원검색(Use JSP/Beans/ResultSet)</div>
			<hr  class="Line">
			<%------------------------------------------------------------------
					조건 입력필드 & 검색 버튼
			----------------------------------------------------------------------%>
		...
		<div class="Middle-Fixed">
			...
			<table class="Table">
				...
				<%
					...
					{
						// 사원정보 출력하기
						while(this.sawonDAO.DBMgr.Rs.next() == true)
						{
							// 성별명 만들기
							sGenderName = this.sawonDAO.DBMgr.Rs.getString("Gender");
							sGenderName = (sGenderName.equals("1")) ? "남성" : "여성";
							
				%>
				<tr class="TableRow">
				<%
							out.println(String.format("<td>%d</td>",	this.sawonDAO.DBMgr.Rs.getInt("Sabun")));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("Name")));
							out.println(String.format("<td>%d</td>",	this.sawonDAO.DBMgr.Rs.getInt("Age")));
							out.println(String.format("<td>%s</td>",	sGenderName));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("Tel")));
							out.println(String.format("<td>[%s]</td>",	this.sawonDAO.DBMgr.Rs.getString("Dept")));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("DeptName")));
							out.println(String.format("<td>[%s]</td>",	this.sawonDAO.DBMgr.Rs.getString("StCd")));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("StCdName")));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("BDate")));
							out.println(String.format("<td>%s</td>",	this.sawonDAO.DBMgr.Rs.getString("Address")));
