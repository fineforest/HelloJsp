...
<head>
	...
	<script type="text/javascript">
		...
		window.onload = function() { LoadContentEditor(); }
		// -----------------------------------------------------------------
		...
		function LoadContentEditor()
		{
			...
			document.getElementById('ContentEditor').innerHTML = ToContentEditorHtml(Content);
		}
        ...
        function SaveNBoardContent() {
            let ContentHtml = document.getElementById('ContentEditor').innerHTML;
            
            document.getElementById('Content').value  = HtmlToContent(ContentHtml);
        }
		// 입력한 게시글 내용에 div가 자동으로 만든 태그를 저장용으로 모두 변환
		// 붙여넣은 이미지 태그<img> 또는 엔터 태그<div><br></div>를 변환
...
switch (sJopStatus)
{
	...
	case "Ex":
		...
		txtContent	= String.format("%s&br;%s&br;%s&br;%s&br;%s&br;&br;%s&br;&br;%s&br;&br;",
									"홍길동은 그 누구보다도 JSP를 좋아 했습니다.",
									"또한 Servlet도 무척 좋아 했습니다.",
									"JSP의 복잡성을 Servlet으로 해결하며 즐거워 하던 어느날,",
									"홍길동은 Mr.빈의 소게로 Java.Bean을 만나게 되었습니다.",
...
<body class="Body">
	...
	<form name="form1" action="NBoardSave.jsp?NBoardPageNum=<%=nNBoardPageNum %>" method="post" enctype="multipart/form-data">
		...
		<table class="Table">
			<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="Author">작성자<span class="Essential">*</span></label></td>
				<td><input class="Data"   type="text" id="Author" name="txtAuthor" value="<%=txtAuthor %>" required/></td>
			</tr>
 			<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="Subject">글제목<span class="Essential">*</span></label></td>
				<td><input class="Data"   type="text" id="Subject" name="txtSubject" value="<%=txtSubject %>" required/></td>
			</tr>
			<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="E-Mail">이메일<span class="Essential">*</span></label></td>
				<td><input class="Data"   type="text" id="E-Mail" name="txtEMail" value="<%=txtEMail %>" required/></td>
			</tr>
<!-- 		<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="Content">글내용<span class="Essential">*</span></label></td>
				<td><textarea class="Content" id="Content" name="txtContent" cols="70" rows="7" required><%=txtContent %></textarea></td>
			</tr>
 -->
			
	 		<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="ContentEditor">글내용<span class="Essential">*</span></label></td>
				<td>
					<div class="ContentEditor" contenteditable="true"  id="ContentEditor"></div>
					<input type="text" id="Content" name="txtContent" value="<%=txtContent %>" style="width:100%"/>
				</td>
			</tr>
			...
			<tr class="TableRow">
				<td class="TableCol"><label class="Label"  for="File">파일첨부</label></td>
				<td>
					<input class="Data"   type="file" id="File" name="txtFile" accept="image/*, video/*, audio/*" onchange="ReadImage(this, 'Preview');"/>
				</td>
			</tr>
			<tr class="TableRow">
				<td class="TableCol"><span class="Label">이미지</span></td>
				<td>
			...
			<tr class="TableRow" align="left">
				<td class="TableCol" colspan="2">
					<input class="Submit" type="submit" value=" 글쓰기저장 " onclick="SaveNBoardContent()"/>
